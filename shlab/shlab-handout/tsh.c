...
*/
void eval(char *cmdline)   // 参考书上的框架
{
    char *argv[MAXARGS];
    char buf[MAXLINE];
    int bg;
    ...
    {   
    	...
    	if ((pid = fork()) == 0)
    	{
            ...
            // 正常运行execve函数会替换内存，不会返回/退出，必须要加exit，
            // 否则会一直运行下去，子进程会运行父进程的代码
